...
bool check_POS_right(int i)
{
    ...
    if(POS[i].find("JJ") != string::npos || POS[i].find("Z99") != string::npos || food_to_tag.count(i) > 0 || is_color[i] == 1 || POS[i].find("NN") != string::npos|| POS[i].find("NP") != string::npos || find(allowed_endings.begin(), allowed_endings.end(), s[i]) != allowed_endings.end())
    {
//        cout << "TRUE:" << endl;
//        cout << "\t" << s[i] << " " << POS[i] << endl << endl;
        return(true);
