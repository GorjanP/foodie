...
Food_checking <- function(s, tagged_data)
{
  ...
  for(i in 1:nrow(tagged_data) )
  {
    ...
    t_temp <- grep("O4.6", tagged_data$sem1[i], value = FALSE)
...
parse_recipe <- function(num, close = TRUE)
{
  ...
  full_files = list.files("allrecipes2/recipes/", full.names = TRUE)
  files = list.files("allrecipes2/recipes/", full.names = FALSE)
