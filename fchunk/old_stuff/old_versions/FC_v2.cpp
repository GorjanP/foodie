...
bool check_POS_left(int i)
{
    ...
    if(POS[i].find("JJ") != string::npos ||  POS[i].find("NP") != string::npos || POS[i].find("Z99") != string::npos || food_to_tag.count(s[i]) != 0 || POS[i].find("NN") != string::npos )
    {
//        cout << "TRUE:" << endl;
...
bool check_POS_right(int i)
{
    ...
    if(POS[i].find("JJ") != string::npos ||  POS[i].find("NP") != string::npos || POS[i].find("VVG") != string::npos || POS[i].find("Z99") != string::npos || food_to_tag.count(s[i]) != 0 || POS[i].find("NN") != string::npos )
    {
//        cout << "TRUE:" << endl;
//        cout << "\t" << s[i] << " " << POS[i] << endl << endl;
        return(true);
...
void join_chunks()
{
    ...
    {
        converted.clear();
        if(food_to_tag.count(s[i]) > 0 && ids[i] == -1)
        {
            ids_ctr++;
            ids[i] = ids_ctr;
...
//            out.open("TESTING/FPNP.txt", ios_base::app);
            ...
            if(POS[last].find("NN") != string::npos && is_obj[last] == 1)
            {
                cout << "Writing.." << endl;
//                out << POS[last] << " - " << s[last] << endl;
...
void find_foods()
{
    ...
    for(int i = 0; i < size_final; i++)
    {
        ...
        while(ss >> zbor)
        {
            ...
            if(food_to_tag.count(zbor) > 0 && ids[id_mapping_final[i][brojac]] > -1)
            {
                found = true;
                break;
            }
